...
// 🚀 โมดูลค้นหาด่วนและลัดคัดลอกข้อมูล (100% Flicker-Free Native 3D Toast Edition)

const SearchCopyPage = {
    html: `
        <style>
    ...
    init: function() {
        ...
        db.ref('patients_database_v2/patients').on('value', snap => {
            ...
            this.allPatients = rawPatients.filter(p => p !== null && p.status !== 'ย้ายคลินิก' && p.status !== 'เสียชีวิต');
            
            const badge = document.getElementById('sc-total-count');
            if (badge) badge.innerHTML = `<i class="fa-solid fa-database me-2"></i> คลังข้อมูลพร้อมค้นหา <b>${this.allPatients.length}</b> ราย`;
            this.triggerSearch();
        });
    ...
    triggerSearch: function() {
        ...
        let html = "";
        filtered.forEach(p => {
            let imgSrc = p.photo_base64 ? (p.photo_base64.startsWith('data:image') ? p.photo_base64 : 'data:image/jpeg;base64,' + p.photo_base64) : 'https://ui-avatars.com/api/?name='+ encodeURIComponent(p.name_th||'X') +'&background=f1f5f9&color=64748b&bold=true';
            let cleanIdCard = (p.idcard || "").replace(/-/g, "");
            ...
            <div class="col-md-6 col-xl-4">
                <div class="modern-panel card-hover-float h-100 p-4 shadow-sm" style="border-radius: 20px;">
                    <div class="d-flex align-items-center mb-4 border-bottom pb-3">
                        ...
                        <div class="min-w-0">
                            <h5 class="fw-bold mb-1 text-dark text-truncate" style="font-family:'Prompt';">${p.title || ''}${p.name_th}</h5>
                            <span class="badge bg-primary fw-bold px-3 py-1 shadow-sm rounded-pill"><i class="fa-solid fa-hospital-user me-1"></i> ${p.hn || '-'}</span>
                        </div>
                    </div>
                    ...
                    <div class="d-flex flex-column">
                        ...
                        <div class="copy-btn-group" style="background: var(--success-light); border: 1px solid #bbf7d0;">
                            ...
                            <button class="btn btn-sm btn-light fw-bold text-success-dark shadow-sm rounded-pill px-4 text-truncate" style="border: 1px solid #86efac; max-width: 170px;" onclick="SearchCopyPage.copyText('${p.hn}', this, 'รหัส HN')">
                                ${p.hn || '-'} <i class="fa-regular fa-copy ms-1"></i>
                            </button>
                        </div>
                        ...
                        <div class="copy-btn-group" style="background: var(--warning-light); border: 1px solid #fde68a;">
                            ...
                            <button class="btn btn-sm btn-light fw-bold text-warning-dark shadow-sm rounded-pill px-4 text-truncate" style="border: 1px solid #fde047; max-width: 170px;" onclick="SearchCopyPage.copyText('${p.name_th}', this, 'ชื่อ-นามสกุล')">
                                ${p.name_th || '-'} <i class="fa-regular fa-copy ms-1"></i>
                            </button>
                        </div>
                    </div>
                </div>
    ...
    // 🌟 ฟังก์ชันยิงแจ้งเตือนอัจฉริยะแบบไร้รอยต่อ (0% Flicker Guaranteed)
    ...
    copyText: function(text, btnElement, fieldName) {
        ...
        navigator.clipboard.writeText(text).then(() => {
            ...
            Swal.fire('ข้อผิดพลาด', 'บราวเซอร์ไม่อนุญาต', 'error');
        });
    }
};