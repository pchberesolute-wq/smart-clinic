...
// 🚀 โมดูลตั้งค่าระบบ (Enterprise Print Engine + Admin PIN Reset Protection)
...
const SettingsPage = {
    ...
    html: '<style>' +
            '.settings-nav-tabs { border-bottom: 2px solid #e2e8f0; gap: 10px; } ' +
            '.settings-nav-tabs .nav-link { border: none; color: var(--muted); font-weight: 600; padding: 12px 24px; border-radius: 12px 12px 0 0; transition: all 0.3s ease; background: transparent; position: relative; font-family: \'Prompt\'; } ' +
            '.settings-nav-tabs .nav-link:hover { color: var(--primary); background: var(--bg-main); } ' +
            '.settings-nav-tabs .nav-link.active { color: var(--primary); background: #fff; box-shadow: 0 -4px 10px rgba(0,0,0,0.02); } ' +
            '.settings-nav-tabs .nav-link.active::after { content: \'\'; position: absolute; bottom: -2px; left: 0; width: 100%; height: 3px; background: var(--primary); border-radius: 3px 3px 0 0; } ' +
            '.settings-nav-tabs .nav-link.text-info.active::after { background: var(--info); } ' +
            '.settings-nav-tabs .nav-link.text-danger.active::after { background: var(--danger); } ' +
            '.price-badge { background: #f0fdf4; color: #166534; border: 1px solid #bbf7d0; font-weight: 700; border-radius: 50px; padding: 4px 10px; font-size: 12px; margin-left: 8px; } ' +
            '.logo-preview-box { width: 100px; height: 100px; object-fit: contain; border: 2px dashed #cbd5e1; border-radius: 12px; background: #f8fafc; display: flex; align-items: center; justify-content: center; overflow: hidden; } ' +
              ...
              '<div class="tab-pane fade show active" id="clinic-panel" role="tabpanel">' +
                  '<div class="modern-panel p-4 pt-5 mb-4" style="border-top: 4px solid var(--primary); border-radius: 16px;">' +
                      ...
                      '<div class="row g-4 position-relative">' +
                          ...
                          '<div class="col-md-12">' +
                              ...
                              '<div class="d-flex align-items-center gap-3">' +
                                  ...
                                  '</div>' +
                                  '<div>' +
                                      '<input type="file" id="file-clinic-logo" class="form-control input-modern mb-2" accept="image/jpeg, image/png, image/webp" onchange="SettingsPage.handleLogoUpload(event, \'hidden-clinic-logo-base64\', \'clinic-logo-preview-container\')">' +
                                      '<div class="d-flex gap-2">' +
                                          '<button class="btn btn-outline-primary fw-bold btn-sm rounded-pill px-3 shadow-sm" onclick="SettingsPage.promptPrintCard(\'clinic\')"><i class="fa-solid fa-print me-1"></i> พิมพ์นามบัตรสถานพยาบาล</button>' +
                                          '<button class="btn btn-outline-danger fw-bold btn-sm rounded-pill px-3 shadow-sm" onclick="SettingsPage.deleteLogo(\'clinic\')"><i class="fa-solid fa-trash me-1"></i> ลบโลโก้</button>' +
                                      '</div>' +
                                  '</div>' +
                              '</div>' +
                      ...
                      '<div class="row g-4 position-relative">' +
                          '<div class="col-md-12">' +
                              ...
                              '<div class="d-flex align-items-center gap-3">' +
                                  ...
                                  '</div>' +
                                  '<div>' +
                                      '<input type="file" id="file-company-logo" class="form-control input-modern mb-2" accept="image/jpeg, image/png, image/webp" onchange="SettingsPage.handleLogoUpload(event, \'hidden-company-logo-base64\', \'company-logo-preview-container\')">' +
                                      '<div class="d-flex gap-2">' +
                                          '<button class="btn btn-outline-warning text-dark fw-bold btn-sm rounded-pill px-4 shadow-sm" onclick="SettingsPage.promptPrintCard(\'company\')"><i class="fa-solid fa-print me-2"></i> พิมพ์นามบัตรใบกำกับภาษี</button>' +
                                          '<button class="btn btn-outline-danger fw-bold btn-sm rounded-pill px-3 shadow-sm" onclick="SettingsPage.deleteLogo(\'company\')"><i class="fa-solid fa-trash me-1"></i> ลบโลโก้</button>' +
                                      '</div>' +
                                  '</div>' +
                              '</div>' +
                      ...
                      '<div class="text-end mt-5 pt-4 border-top">' +
                          '<button class="btn btn-premium btn-premium-primary px-5" onclick="SettingsPage.saveClinicInfo()">' +
              ...
              '<div class="tab-pane fade" id="users-panel" role="tabpanel">' +
                  '<div class="modern-panel p-4" style="border-top: 4px solid var(--info); border-radius: 16px;">' +
                      ...
                      '<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3 position-relative z-1">' +
                          ...
                          '<div class="d-flex gap-2">' +
                              // 🚨 [NEW] ปุ่มตั้งค่า PIN สำหรับรีเซ็ตรหัสผ่าน 🚨
                              '<button class="btn btn-outline-danger fw-bold shadow-sm rounded-pill px-4" onclick="SettingsPage.setAdminPin()"><i class="fa-solid fa-key me-2"></i> ตั้งค่า PIN รีเซ็ตรหัส</button>' +
                              '<button class="btn btn-premium btn-premium-primary text-white fw-bold shadow-sm rounded-pill px-4" onclick="SettingsPage.openUserModal()"><i class="fa-solid fa-user-plus me-2"></i> เพิ่มผู้ใช้งานใหม่</button>' +
                          '</div>' +
                      '</div>' +
              ...
              '<div class="tab-pane fade" id="medical-panel" role="tabpanel">' +
                  '<div class="modern-panel mb-4 p-4" style="border-top: 4px solid var(--danger); border-radius: 16px;">' +
                      '<div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">' +
                          ...
                          '<button class="btn btn-outline-danger fw-bold shadow-sm rounded-pill px-4" onclick="SettingsPage.openLabSetModal()"><i class="fa-solid fa-plus me-1"></i> สร้างชุดแล็บใหม่</button>' +
                      '</div>' +
                      ...
                      '<div class="col-xl-6">' +
                          '<div class="modern-panel p-4 h-100" style="border-top: 4px solid #8b5cf6; border-radius: 16px;">' +
                              '<div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">' +
                                  ...
                                  '<button class="btn text-white fw-bold shadow-sm rounded-pill px-3" style="background:#8b5cf6;" onclick="SettingsPage.openMedListModal()"><i class="fa-solid fa-plus me-1"></i> เพิ่มรายการ</button>' +
                              '</div>' +
                      ...
                      '<div class="col-xl-6">' +
                          '<div class="modern-panel p-4 h-100" style="border-top: 4px solid #14b8a6; border-radius: 16px;">' +
                              '<div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">' +
                                  ...
                                  '<button class="btn text-white fw-bold shadow-sm rounded-pill px-3" style="background:#14b8a6;" onclick="SettingsPage.openXrayModal()"><i class="fa-solid fa-plus me-1"></i> เพิ่มรายการ</button>' +
                              '</div>' +
                  ...
                  '<div class="modern-panel mb-4 p-4" style="border-top: 4px solid var(--warning); border-radius: 16px;">' +
                      '<div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">' +
                          ...
                          '<button class="btn btn-outline-warning text-dark fw-bold shadow-sm rounded-pill px-4" onclick="SettingsPage.openNoteTemplateModal()"><i class="fa-solid fa-plus me-1"></i> สร้างเทมเพลต</button>' +
                      '</div>' +
                      '<ul class="nav nav-pills mb-4 p-2 bg-light rounded-pill flex-wrap" id="note-category-tabs" role="tablist" style="gap: 5px; display: inline-flex;">' +
                          '<li class="nav-item"><button class="nav-link active fw-bold px-4 rounded-pill" onclick="SettingsPage.filterNotes(\'all\')"><i class="fa-solid fa-layer-group me-1"></i> ทั้งหมด</button></li>' +
                          '<li class="nav-item"><button class="nav-link fw-bold px-3 rounded-pill text-primary" onclick="SettingsPage.filterNotes(\'pre\')">อาการก่อนฟอก</button></li>' +
                          '<li class="nav-item"><button class="nav-link fw-bold px-3 rounded-pill text-danger" onclick="SettingsPage.filterNotes(\'intra\')">แทรกซ้อน (Intra)</button></li>' +
                          '<li class="nav-item"><button class="nav-link fw-bold px-3 rounded-pill text-success" onclick="SettingsPage.filterNotes(\'post\')">สรุปหลังฟอก</button></li>' +
                          '<li class="nav-item"><button class="nav-link fw-bold px-3 rounded-pill text-info" onclick="SettingsPage.filterNotes(\'doctor\')"><i class="fa-solid fa-user-doctor me-1"></i> คำสั่งแพทย์</button></li>' +
                      '</ul>' +
    ...
    allUsers: [], labSets: [], noteTemplates: [], currentNoteFilter: 'all', medsList: [], xraysList: [],
    ...
    init: function() {
        ...
        db.ref('clinic_users_v2').on('value', snap => { this.allUsers = toArray(snap.val()); this.renderUsersTable(); });

        db.ref('clinic_lab_sets_v2').on('value', snap => { 
            let _labs = toArray(snap.val());
            this.labSets = _labs.length > 0 ? _labs : [{ id: 'LS1', name: 'Monthly Lab (ประจำเดือน)', price: 1500, items: ['BUN', 'Cr', 'K', 'Ca', 'P', 'Hct'] }]; 
            this.renderLabSets(); 
        });
        
        db.ref('clinic_meds_list_v2').on('value', snap => { 
            let _meds = toArray(snap.val());
            this.medsList = _meds.length > 0 ? _meds : [{ id: 'M1', name: 'Erythropoietin (EPO) 4000U', price: 300 }, { id: 'M2', name: 'Iron Sucrose 100mg', price: 150 }]; 
            this.renderMedsList(); 
        });

        db.ref('clinic_xray_list_v2').on('value', snap => { 
            let _xrays = toArray(snap.val());
            this.xraysList = _xrays.length > 0 ? _xrays : [{ id: 'X1', name: 'CXR (Chest X-Ray)', price: 350 }]; 
            this.renderXrayList(); 
        });
        ...
        db.ref('clinic_note_templates_v2').on('value', snap => {
            ...
            this.noteTemplates = _notes.length > 0 ? _notes.map(n => ({...n, category: n.category || 'general'})) : defaultNotes; 
            this.renderNoteTemplates();
        });
    },

    // 🚨 [NEW] ฟังก์ชันสำหรับให้ Admin ตั้งค่า PIN เพื่อใช้รีเซ็ตรหัสผ่าน
    setAdminPin: function() {
        Swal.fire({
    ...
    handleLogoUpload: function(event, hiddenInputId, previewContainerId) {
    ...
    generateWatermarkImage: function(base64) {
    ...
    executePrintCards: async function(type, qty, themeClass) {
        ...
        let cardsHtml = '';
        for (let i = 0; i < qty; i++) {
            cardsHtml += '<div class="business-card ' + themeClass + '">' +
                    ...
                    '<div class="header-section">' +
                        ...
                        '<div class="title-wrapper">' +
                            '<div class="company-name-container">' +
                                '<div class="company-name-text">' + cardName + '</div>' +
                            '</div>' +
                            '<div class="tax-badge-wrapper">' +
                                '<div class="tax-badge">' + idLabel + ': ' + idValue + '</div>' +
                            '</div>' +
                        '</div>' +
                    '</div>' +
                    '<div class="contact-info">' +
                        '<div class="info-row"><div class="info-icon"><i class="fa-solid fa-location-dot"></i></div><div class="info-text">' + cardAddress + '</div></div>' +
                        '<div class="info-row mt-1"><div class="info-icon"><i class="fa-solid fa-phone"></i></div><div class="info-text">' + cardPhone + (cardEmail !== '-' ? ' &nbsp;|&nbsp; ' + cardEmail : '') + '</div></div>' +
                    '</div>' +
                '</div>' +
            '</div>';
        }
        ...
        const printContent = '<html lang="th">' +
            '<head>' +
                '<meta charset="UTF-8">' +
            ...
            '</head>' +
            '<body>' +
                '<div class="instruction"><h4 style="margin:0 0 5px 0; color:#0f172a;"><i class="fa-solid fa-print text-primary"></i> โหมดพรีวิวการพิมพ์</h4><span style="color:#ef4444; font-weight:bold;">🚨 คำเตือน:</span> ก่อนกดพิมพ์ ให้ดูที่การตั้งค่าเครื่องพิมพ์ แล้วติ๊กถูกที่ช่อง <b>"พิมพ์กราฟิกพื้นหลัง" (Background graphics)</b> ด้วยนะครับ 🖨️ (ปรับ Scale = 100%)</div>' +
    ...
    renderUsersTable: function() {
        ...
        if(!tbody) return;
        if(this.allUsers.length === 0) { tbody.innerHTML = '<tr><td colspan="5" class="text-center py-5 text-muted"><i class="fa-solid fa-users-slash fa-3x mb-3" style="opacity:0.2;"></i><br>ยังไม่มีข้อมูลผู้ใช้งาน</td></tr>'; return; }
        let html = '';
        this.allUsers.forEach(user => {
            ...
            let imgSrc = 'https://ui-avatars.com/api/?name=' + encodeURIComponent(user.name||'U') + '&background=f8fafc&color=334155&bold=true';
            
            html += '<tr class="card-hover-float" style="cursor: default;">' +
                '<td><div class="d-flex align-items-center"><div class="position-relative"><img src="' + imgSrc + '" class="rounded-circle me-3 shadow-sm border border-2 border-white" style="width: 42px; height: 42px;">' + (user.status === 'active' ? '<span class="position-absolute bottom-0 end-0 p-1 bg-success border border-2 border-white rounded-circle" style="transform: translate(-10px, -2px);"></span>' : '') + '</div><div><div class="fw-bold text-dark" style="font-size: 15px;">' + user.name + '</div><div class="small text-muted" style="font-size: 12px;">ID: ' + user.id + '</div></div></div></td>' +
                '<td class="fw-bold text-primary" style="font-size: 14.5px;">' + user.username + '</td>' +
                '<td class="text-center">' + roleBadge + '</td>' +
                '<td class="text-center">' + statusBadge + '</td>' +
                '<td class="text-center">' +
                    '<button class="btn btn-sm btn-light border shadow-sm me-1 fw-bold text-dark" onclick="SettingsPage.openUserModal(\'' + user.id + '\')" title="แก้ไข"><i class="fa-solid fa-pen"></i></button>' +
                    '<button class="btn btn-sm btn-outline-danger shadow-sm" onclick="SettingsPage.deleteUser(\'' + user.id + '\')" title="ลบ"><i class="fa-solid fa-trash"></i></button>' +
                '</td></tr>';
        });
    ...
    openUserModal: function(userId = null) {
        let isEdit = !!userId; let user = isEdit ? this.allUsers.find(u => u.id === userId) : { status: 'active', role: 'nurse' };
        ...
        Swal.fire({
            title: '<h4 class="text-primary fw-bold mb-0"><i class="fa-solid ' + (isEdit?'fa-user-pen':'fa-user-plus') + ' me-2"></i>' + (isEdit ? 'แก้ไขข้อมูลผู้ใช้งาน' : 'ลงทะเบียนผู้ใช้ใหม่') + '</h4>', width: 600,
            html: '<div class="text-start px-2 mt-3" style="font-family:\'Sarabun\';"><div class="row g-3">' +
                '<div class="col-12"><label class="fw-bold text-secondary small">ชื่อ - นามสกุล <span class="text-danger">*</span></label><input type="text" id="swal-user-name" class="form-control input-modern" value="' + (user.name || '') + '"></div>' +
                '<div class="col-6"><label class="fw-bold text-secondary small">Username (ไอดีล็อคอิน) <span class="text-danger">*</span></label><input type="text" id="swal-user-username" class="form-control input-modern text-primary" value="' + (user.username || '') + '" ' + (isEdit?'readonly':'') + '></div>' +
                '<div class="col-6"><label class="fw-bold text-secondary small">รหัสผ่าน ' + (isEdit?'':'<span class="text-danger">*</span>') + '</label><input type="password" id="swal-user-password" class="form-control input-modern" placeholder="' + (isEdit?'เว้นว่างถ้าไม่เปลี่ยน':'ตั้งรหัสผ่าน 6 ตัวขึ้นไป') + '"></div>' +
                '<div class="col-6"><label class="fw-bold text-secondary small">ระดับสิทธิ์ (Role)</label><select id="swal-user-role" class="form-select input-modern">' + roleOptionsHtml + '</select></div>' +
                '<div class="col-6"><label class="fw-bold text-secondary small">สถานะบัญชี</label><select id="swal-user-status" class="form-select input-modern"><option value="active" ' + (user.status==='active'?'selected':'') + '>🟢 ใช้งานปกติ</option><option value="inactive" ' + (user.status==='inactive'?'selected':'') + '>🔴 ระงับการใช้งาน</option></select></div>' +
            '</div></div>',
            ...
            preConfirm: () => {
                const name = document.getElementById('swal-user-name').value.trim(); const username = document.getElementById('swal-user-username').value.trim(); const password = document.getElementById('swal-user-password').value.trim();
                ...
                return { id: userId || 'USR-'+Date.now(), name, username, password: password ? password : user.password, role: document.getElementById('swal-user-role').value, status: document.getElementById('swal-user-status').value };
            }
        }).then((result) => { if (result.isConfirmed) { let updatedList = [...this.allUsers]; if (isEdit) updatedList[updatedList.findIndex(u => u.id === userId)] = result.value; else updatedList.push(result.value); db.ref('clinic_users_v2').set(updatedList).then(() => Swal.fire({title:'บันทึกสำเร็จ!', icon:'success', timer:1200, showConfirmButton:false})); } });
    },
    
    deleteUser: function(userId) {
        const user = this.allUsers.find(u => u.id === userId);
        if(user.role === 'admin' && this.allUsers.filter(u => u.role === 'admin').length === 1) { Swal.fire('ลบไม่ได้!', 'ต้องมี Admin อย่างน้อย 1 คน', 'error'); return; }
        Swal.fire({ title: 'ยืนยันการลบ?', text:'ระงับสิทธิ์ของ ' + user.name + ' ใช่หรือไม่?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', confirmButtonText: '<i class="fa-solid fa-trash me-1"></i> ลบทิ้ง', cancelButtonText: 'ยกเลิก' }).then((res) => { if(res.isConfirmed) db.ref('clinic_users_v2').set(this.allUsers.filter(u => u.id !== userId)); });
    },

    renderLabSets: function() {
        const container = document.getElementById('lab-sets-container');
        if(!container) return;
        if(this.labSets.length === 0) { container.innerHTML = '<div class="col-12 text-center text-muted"><i class="fa-solid fa-vial fa-3x mb-3" style="opacity:0.2;"></i><br>ยังไม่มีชุดแล็บ</div>'; return; }

        let html = '';
        this.labSets.forEach(s => {
            let tags = s.items.map(i => '<span class="badge bg-light text-dark border px-2 py-1 me-1 mb-1 shadow-sm"><i class="fa-solid fa-droplet text-danger me-1"></i>' + i + '</span>').join('');
            ...
            html += '<div class="col-md-6 col-lg-4">' +
                '<div class="modern-panel card-hover-float p-3 h-100 d-flex flex-column position-relative" style="border-left: 4px solid var(--danger);">' +
                    '<h6 class="fw-bold text-dark mb-3"><i class="fa-solid fa-folder-open text-danger me-2"></i> ' + s.name + ' ' + priceLabel + '</h6>' +
                    '<div class="mb-3">' + tags + '</div>' +
                    '<div class="mt-auto text-end border-top pt-3">' +
                        '<button class="btn btn-sm btn-light border shadow-sm fw-bold px-3 text-dark me-1" onclick="SettingsPage.openLabSetModal(\'' + s.id + '\')">แก้ไข</button>' +
                        '<button class="btn btn-sm btn-outline-danger shadow-sm px-3" onclick="SettingsPage.deleteLabSet(\'' + s.id + '\')"><i class="fa-solid fa-trash"></i></button>' +
                    '</div>' +
                '</div>' +
            '</div>';
        });
    ...
    openLabSetModal: function(setId = null) {
        let isEdit = !!setId;
        let set = isEdit ? this.labSets.find(s => s.id === setId) : { items: [], price: 0 };
        ...
        Swal.fire({
            ...
            html: '<div class="text-start mt-3" style="font-family:\'Sarabun\';">' +
                    ...
                    '<input type="text" id="swal-lab-name" class="form-control input-modern mb-3" value="' + (set.name || '') + '" placeholder="เช่น เจาะเลือดประจำเดือน">' +
            ...
            if(res.isConfirmed) {
                let updated = [...this.labSets];
                if(isEdit) updated[updated.findIndex(s=>s.id===setId)] = res.value; else updated.push(res.value);
                db.ref('clinic_lab_sets_v2').set(updated);
            }
        });
    },
    
    deleteLabSet: function(id) { db.ref('clinic_lab_sets_v2').set(this.labSets.filter(s=>s.id!==id)); },

    renderMedsList: function() {
        const container = document.getElementById('meds-list-container');
        if(!container) return;
        let html = '';
        this.medsList.forEach(m => {
            let priceHtml = m.price ? '<span class="badge bg-white text-primary border ms-2">฿' + Number(m.price).toLocaleString() + '</span>' : '';
            html += '<span class="badge bg-light text-dark border border-secondary-subtle px-3 py-2 fs-6 shadow-sm card-hover-float" style="border-radius:12px; font-weight: 600;">' +
                        '<i class="fa-solid fa-pills text-primary me-1"></i> <span onclick="SettingsPage.openMedListModal(\'' + m.id + '\')" style="cursor:pointer;">' + m.name + '</span> ' + priceHtml +
                        '<i class="fa-solid fa-times ms-3 text-danger" style="cursor:pointer;" onclick="SettingsPage.deleteMedItem(\'' + m.id + '\')" title="ลบรายการ"></i>' +
                     '</span>';
        });
    ...
    openMedListModal: function(id = null) {
        let isEdit = !!id;
        let m = isEdit ? this.medsList.find(x => x.id === id) : { name: '', price: '' };
        Swal.fire({
            ...
            html: '<div class="text-start mt-3" style="font-family:\'Sarabun\';">' +
                    ...
                    '<input type="text" id="swal-med-name" class="form-control input-modern mb-3 fw-bold text-dark" value="' + m.name + '" placeholder="เช่น Heparin 5000U">' +
            ...
            if(res.isConfirmed) {
                let updated = [...this.medsList];
                if(isEdit) updated[updated.findIndex(x=>x.id===id)] = res.value; else updated.push(res.value);
                db.ref('clinic_meds_list_v2').set(updated);
            }
        });
    },
    
    deleteMedItem: function(id) { db.ref('clinic_meds_list_v2').set(this.medsList.filter(x=>x.id!==id)); },

    renderXrayList: function() {
        const container = document.getElementById('xrays-list-container');
        if(!container) return;
        let html = '';
        this.xraysList.forEach(x => {
            let priceHtml = x.price ? '<span class="badge bg-white text-info border ms-2">฿' + Number(x.price).toLocaleString() + '</span>' : '';
            html += '<span class="badge bg-light text-dark border border-secondary-subtle px-3 py-2 fs-6 shadow-sm card-hover-float" style="border-radius:12px; font-weight: 600;">' +
                        '<i class="fa-solid fa-x-ray text-info me-1"></i> <span onclick="SettingsPage.openXrayModal(\'' + x.id + '\')" style="cursor:pointer;">' + x.name + '</span> ' + priceHtml +
                        '<i class="fa-solid fa-times ms-3 text-danger" style="cursor:pointer;" onclick="SettingsPage.deleteXrayItem(\'' + x.id + '\')" title="ลบรายการ"></i>' +
                     '</span>';
        });
        container.innerHTML = html || '<span class="text-muted small"><i class="fa-solid fa-box-open me-1"></i> ยังไม่มีรายการเอ็กซเรย์</span>';
    },

    openXrayModal: function(id = null) {
        let isEdit = !!id;
        let x = isEdit ? this.xraysList.find(i => i.id === id) : { name: '', price: '' };
        Swal.fire({
            ...
            html: '<div class="text-start mt-3" style="font-family:\'Sarabun\';">' +
                    ...
                    '<input type="text" id="swal-xray-name" class="form-control input-modern mb-3 fw-bold text-dark" value="' + x.name + '" placeholder="เช่น Chest X-Ray (CXR)">' +
            ...
            if(res.isConfirmed) {
                let updated = [...this.xraysList];
                if(isEdit) updated[updated.findIndex(i=>i.id===id)] = res.value; else updated.push(res.value);
                db.ref('clinic_xray_list_v2').set(updated);
            }
        });
    },
    
    deleteXrayItem: function(id) { db.ref('clinic_xray_list_v2').set(this.xraysList.filter(x=>x.id!==id)); },

    filterNotes: function(cat) {
        this.currentNoteFilter = cat;
    ...
    renderNoteTemplates: function() {
        const container = document.getElementById('note-templates-container');
        if(!container) return;

        let filteredNotes = this.noteTemplates;
        if(this.currentNoteFilter !== 'all') filteredNotes = this.noteTemplates.filter(t => t.category === this.currentNoteFilter);
        if(filteredNotes.length === 0) { container.innerHTML = '<div class="col-12 text-center text-muted"><i class="fa-regular fa-file-lines fa-3x mb-3" style="opacity:0.2;"></i><br>ไม่พบเทมเพลตในหมวดหมู่นี้</div>'; return; }
            ...
            html += '<div class="col-md-6 col-lg-4">' +
                '<div class="modern-panel card-hover-float p-3 h-100 d-flex flex-column position-relative" style="border-top: 4px solid ' + borderColor + '; border-radius: 14px;">' +
                    '<div class="d-flex justify-content-between align-items-start mb-3">' +
                        '<h6 class="fw-bold text-dark mb-0" style="padding-right: 60px; line-height: 1.4;">' + iconHtml + ' ' + t.title + '</h6>' +
                        '<div style="position: absolute; right: 15px; top: 15px;">' + catBadge + '</div>' +
                    '</div>' +
                    '<p class="text-muted small mb-3 p-2 bg-light rounded" style="display:-webkit-box; -webkit-line-clamp:3; -webkit-box-orient:vertical; overflow:hidden; white-space: pre-wrap; font-size: 13px;">' + t.text + '</p>' +
                    '<div class="text-end mt-auto pt-2">' +
                        '<button class="btn btn-sm btn-light border text-dark fw-bold px-3 shadow-sm me-1" onclick="SettingsPage.openNoteTemplateModal(\'' + t.id + '\')">แก้ไข</button>' +
                        '<button class="btn btn-sm btn-outline-danger shadow-sm px-3" onclick="SettingsPage.deleteNoteTemplate(\'' + t.id + '\')"><i class="fa-solid fa-trash"></i></button>' +
                    '</div>' +
                '</div>' +
            '</div>';
        });
    ...
    openNoteTemplateModal: function(id = null) {
        let isEdit = !!id;
        let t = isEdit ? this.noteTemplates.find(x => x.id === id) : { category: 'general' };
        ...
        Swal.fire({
            ...
            html: '<div class="text-start mt-3" style="font-family:\'Sarabun\';">' +
                    ...
                    '<input type="text" id="swal-note-title" class="form-control input-modern mb-3" value="' + (t.title || '') + '" placeholder="เช่น ความดันตกให้ NSS">' +
                    '<label class="fw-bold text-secondary small">ข้อความเนื้อหาที่จะบันทึกลงระบบ</label>' +
                    '<textarea id="swal-note-text" class="form-control input-modern p-3" rows="5" placeholder="พิมพ์ข้อความมาตรฐานที่นี่...">' + (t.text || '') + '</textarea>' +
                '</div>',
            ...
            if(res.isConfirmed) {
                let updated = [...this.noteTemplates];
                if(isEdit) updated[updated.findIndex(x=>x.id===id)] = res.value; else updated.push(res.value);
    ...
    deleteNoteTemplate: function(id) { db.ref('clinic_note_templates_v2').set(this.noteTemplates.filter(x=>x.id!==id)); }
};