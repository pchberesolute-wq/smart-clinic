...
const UsageStatisticsPage = {
        ...
        <div class="modern-panel shadow-sm p-4 position-relative overflow-hidden" style="border-top:5px solid var(--secondary); border-radius:20px; background:#fff; border:1px solid #e2e8f0;">
            <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
                ...
                <div class="search-box-modern shadow-sm bg-light" style="width: 250px;">
                    ...
                    <input type="text" id="stat-search-inp" class="border-0 bg-transparent ms-2 w-100 fw-bold text-dark" placeholder="ค้นหาชื่อ, รหัส..." onkeyup="UsageStatisticsPage.filterTable()" style="outline:none;">
                </div>
            </div>
    ...
    updateDateDisplays: function() {
        if(document.getElementById('stat-start-date')) document.getElementById('stat-start-date').value = this.startDate;
        if(document.getElementById('stat-end-date')) document.getElementById('stat-end-date').value = this.endDate;
        if(document.getElementById('stat-start-display')) document.getElementById('stat-start-display').innerText = this.formatDateTh(this.startDate);
        if(document.getElementById('stat-end-display')) document.getElementById('stat-end-display').innerText = this.formatDateTh(this.endDate);
    },
    ...
    init: function() {
        ...
        let domBinder = setInterval(() => {
            if(document.getElementById('stat-start-display')) { 
        ...
        db.ref('inventory_database_v2/items').once('value', snap => {
            ...
            db.ref('inventory_database_v2/transactions').on('value', tSnap => {
                if (!document.getElementById('stat-table-body')) { 
                    try { db.ref('inventory_database_v2/transactions').off(); } catch(e){} 
                    return; 
                }
                const transData = tSnap.val(); 
                this.allTransactions = transData ? Object.keys(transData).map(k => ({ id: k, ...transData[k] })) : [];
                this.processData();
            });
        });
    },
    ...
    changeView: function(vType) { 
        this.selectedView = vType; 
        this.syncActiveButton(); 
        this.processData(); 
    },
    
    syncActiveButton: function() { 
        ...
        const activeBtn = document.getElementById('btn-v-' + this.selectedView); 
    ...
    processData: function() {
        ...
        let filteredTrans = this.allTransactions.filter(t => {
            if (!t.timestamp) return false; 
            let dp = t.timestamp.split('T')[0];
            return dp >= this.startDate && dp <= this.endDate && (t.mode === 'out_sub' || t.mode === 'out');
        });
        ...
        filteredTrans.forEach(t => {
            ...
            if (this.selectedView === 'daily') pKey = t.timestamp.split('T')[0];
            else if (this.selectedView === 'weekly') pKey = this.getWeekNumber(dateObj);
            else if (this.selectedView === 'monthly') pKey = t.timestamp.substring(0, 7);
            else if (this.selectedView === 'yearly') pKey = t.timestamp.substring(0, 4);
        ...
        Object.keys(periodGroups).sort().forEach(pKey => {
            ...
            else if(this.selectedView === 'yearly') { dp = 'พ.ศ. ' + (Number(pKey)+543); } 
            else if(this.selectedView === 'daily') { dp = this.formatDateTh(pKey).substring(0,5); }
            cLabels.push(dp);
        ...
        Object.keys(periodGroups).sort().reverse().forEach(pKey => {
            ...
            Object.keys(itemsInPeriod).forEach(iId => {
                ...
                else if(this.selectedView === 'yearly') { dp = 'ปี พ.ศ. ' + (Number(pKey)+543); } 
                else if(this.selectedView === 'daily') { dp = this.formatDateTh(pKey); }
        ...
        this._chartPayload = { labels: cLabels, fluid: cFluidData, meds: cMedsData };
        this._piePayload = { labels: top5Names, data: top5Qtys };
        this.renderChart();
    },
    ...
    renderChart: function() {
        if (!this._chartPayload) return;
        
        const ctxBar = document.getElementById('usageStatisticsChart');
        if (ctxBar) {
            if (this.chartInstance) this.chartInstance.destroy();
            this.chartInstance = new Chart(ctxBar.getContext('2d'), {
                type: 'bar',
                data: {
                    labels: this._chartPayload.labels,
                    datasets: [
                        { label: 'น้ำยาไตเทียม (แกลลอน/ชิ้น)', data: this._chartPayload.fluid, backgroundColor: 'rgba(16, 185, 129, 0.85)', borderColor: '#10b981', borderWidth: 2, borderRadius: 6 },
                        { label: 'ยาและเวชภัณฑ์ (Vial/ชิ้น)', data: this._chartPayload.meds, backgroundColor: 'rgba(239, 68, 68, 0.85)', borderColor: '#ef4444', borderWidth: 2, borderRadius: 6 }
                    ]
                },
        ...
        if (ctxPie && this._piePayload.labels.length > 0) {
            if (this.pieChartInstance) this.pieChartInstance.destroy();
            this.pieChartInstance = new Chart(ctxPie.getContext('2d'), {
                type: 'doughnut',
                data: {
                    labels: this._piePayload.labels,
                    datasets: [{ data: this._piePayload.data, backgroundColor: ['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6'], hoverOffset: 4 }]
                },
    ...
    printReport: function() {
        this.renderChart();
        if(!this._chartPayload) { Swal.fire('ข้อมูลว่างเปล่า', 'ไม่มีประวัติสถิติเพื่อจัดพิมพ์รายงาน', 'warning'); return; }
        ...
        db.ref('clinic_settings_v2').once('value').then(snap => {
            ...
            if (chartCanvas && this.chartInstance) { 
            ...
            if (pieCanvas && this.pieChartInstance) { 
            ...
            let viewTitle = this.selectedView === 'daily' ? 'รายวัน' : this.selectedView === 'weekly' ? 'รายสัปดาห์' : this.selectedView === 'monthly' ? 'รายเดือน' : 'รายปี';
            ...
            let htmlContent = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>รายงานสถิติพัสดุ</title><link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@400;600;700&display=swap" rel="stylesheet">' +
                ...
                '</head><body>' +
                '<div class="header"><h2>' + settings.clinic_name + '</h2><h3>รายงานวิเคราะห์สถิติปริมาณการเบิกใช้น้ำยาและยาฉีดเวชภัณฑ์ (' + viewTitle + ')</h3><div style="margin-top:5px;">รอบการประมวลผล: ' + this.formatDateTh(this.startDate) + ' ถึง ' + this.formatDateTh(this.endDate) + '</div></div>' +
