...
// 🚀 Enterprise Flowsheet Module: Atomic Save, FinOps Fetching & Memory Leak Guard

class VisitDetailPageComponent {
    constructor() {
        this.state = {
            visitId: null,
            visitFirebaseKey: null, // 🔥 สำคัญ: กุญแจชี้เป้าสำหรับ Update ข้อมูลคิว
            patientFirebaseKey: null, // 🔥 สำคัญ: กุญแจชี้เป้าสำหรับ Update ประวัติคนไข้
            visitData: null,
            isFormLoaded: false,
            inventoryItems: new Map(), // Hash Map เพื่อความเร็วแสง
            labSets: [], noteTemplates: [], medsList: [], xraysList: [], modalities: [], clinicRights: [],
            clinicSettings: {},
            
            // Temporary State (การแก้ไขปัจจุบันก่อนเซฟ)
        ...
        return `
            ...
            <div id="vd-main-screen" style="display: none;">
                <div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
                    <div>
                        <button class="btn btn-light shadow-sm fw-bold rounded-pill text-dark px-4 py-2 border card-hover-float" onclick="App.pages.visit_detail.init(null)"><i class="fa-solid fa-arrow-left me-2 text-primary"></i> เปลี่ยนผู้ป่วย</button>
                    ...
                    <div><button class="btn btn-premium btn-premium-success px-4 py-2" style="font-size: 15px;" onclick="App.pages.visit_detail.saveData()"><i class="fa-solid fa-cloud-arrow-up me-2"></i> บันทึกเข้าระบบ EMR</button></div>
                </div>
                        ...
                        <div class="col-xl-4 col-lg-6">
                            <div class="compact-panel modern-panel h-100 position-relative overflow-hidden" style="border-top: 5px solid var(--info);">
                                <div class="section-title-compact text-info">
                                    ...
                                    <button class="btn btn-sm btn-outline-info rounded-pill fw-bold shadow-sm px-3 py-1 bg-white" onclick="App.pages.visit_detail.pullPreviousData('machine')"><i class="fa-solid fa-clock-rotate-left me-1"></i> ดึงประวัติ</button>
                                </div>
                                <div class="row g-3">
                                    ...
                                    <div class="col-6"><div class="d-flex justify-content-between align-items-end mb-1"><label class="form-label fw-bold text-secondary small mb-0">โหมดฟอก</label><button class="btn btn-sm text-primary p-0 fw-bold" onclick="App.pages.visit_detail.manageModalities()"><i class="fa-solid fa-gear"></i></button></div><select id="vd-mode" class="form-select input-modern fw-bold text-dark" onchange="App.pages.visit_detail.onModeChange()"></select></div>
                                    <div class="col-6"><div class="d-flex justify-content-between align-items-end mb-1"><label class="form-label fw-bold text-secondary small mb-0">สิทธิรักษา</label><button class="btn btn-sm text-success p-0 fw-bold" onclick="App.pages.visit_detail.manageRights()"><i class="fa-solid fa-gear"></i></button></div><select id="vd-right" class="form-select input-modern fw-bold text-success" onchange="App.pages.visit_detail.onRightChange()" style="border-color:#86efac; background-color:#f0fdf4;"></select></div>
                                ...
                                <div class="section-title-compact text-danger">
                                    ...
                                    <button class="btn btn-sm btn-outline-danger rounded-pill fw-bold shadow-sm px-3 py-1 bg-white" onclick="App.pages.visit_detail.pullPreviousData('vitals')"><i class="fa-solid fa-clock-rotate-left me-1"></i> ดึงประวัติ</button>
                                </div>
                                <div class="row g-3">
                            ...
                            <div class="compact-panel modern-panel h-100 position-relative overflow-hidden" style="border-top: 5px solid var(--warning);">
                                <div class="section-title-compact text-warning-dark">
                                    ...
                                    <div class="d-flex gap-2 align-items-center">
                                        ...
                                        <button class="btn btn-sm btn-outline-warning text-warning-dark rounded-pill fw-bold shadow-sm px-3 py-1 bg-white" onclick="App.pages.visit_detail.pullPreviousData('main_meds')"><i class="fa-solid fa-clock-rotate-left me-1"></i> ดึงยาหลัก</button>
                                    </div>
                                </div>
                                <div class="row g-3 mb-4">
                                    <div class="col-6"><label class="form-label fw-bold text-secondary small mb-1">น้ำยาไต (Dialysate)</label><select id="vd-dialysate-item" class="form-select input-modern mb-2 fw-bold text-dark" onchange="App.pages.visit_detail.calculateAdditionalFees(true)"></select><div class="solid-input-group"><span class="sig-addon sig-prepend px-2">จำนวน</span><input type="number" id="vd-dialysate-qty" class="text-primary text-center" placeholder="แกลลอน" onkeyup="App.pages.visit_detail.calculateAdditionalFees(true)" onchange="App.pages.visit_detail.calculateAdditionalFees(true)"></div></div>
                                    <div class="col-6"><label class="form-label fw-bold text-secondary small mb-1">น้ำเกลือ (NSS)</label><select id="vd-saline-item" class="form-select input-modern mb-2 fw-bold text-dark" onchange="App.pages.visit_detail.calculateAdditionalFees(true)"></select><div class="solid-input-group"><span class="sig-addon sig-prepend px-2">จำนวน</span><input type="number" id="vd-saline-qty" class="text-primary text-center" placeholder="ขวด/ถุง" onkeyup="App.pages.visit_detail.calculateAdditionalFees(true)" onchange="App.pages.visit_detail.calculateAdditionalFees(true)"></div></div>
                                    <div class="col-12"><label class="form-label fw-bold text-secondary small mb-1">ยาต้านแข็งตัว (Heparin)</label><div class="row g-2"><div class="col-8"><select id="vd-heparin-item" class="form-select input-modern fw-bold text-dark" onchange="App.pages.visit_detail.calculateAdditionalFees(true)"></select></div><div class="col-4"><div class="solid-input-group"><span class="sig-addon sig-prepend px-2">จน.</span><input type="number" id="vd-heparin-qty" class="text-primary text-center" placeholder="Vial" onkeyup="App.pages.visit_detail.calculateAdditionalFees(true)" onchange="App.pages.visit_detail.calculateAdditionalFees(true)"></div></div></div></div>
                                </div>
                                ...
                                <div class="p-3 bg-light rounded-4 border border-secondary-subtle mb-3 shadow-sm">
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        ...
                                        <button class="btn btn-sm btn-outline-primary rounded-pill fw-bold shadow-sm px-3 py-1 bg-white" onclick="App.pages.visit_detail.pullPreviousData('other_meds')"><i class="fa-solid fa-clock-rotate-left me-1"></i> ดึงยาเก่า</button>
                                    </div>
                                    <div id="vd-other-meds-container" class="mb-3"></div>
                                    <button class="btn btn-outline-primary fw-bold rounded-pill w-100 shadow-sm py-2 bg-white" id="btn-add-med" onclick="App.pages.visit_detail.addOtherMed()">+ เพิ่มรายการยา / เวชภัณฑ์</button>
                                </div>
                                ...
                                <div class="p-3 bg-light rounded-4 border border-info-subtle mb-4 shadow-sm">
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        ...
                                        <button class="btn btn-sm btn-outline-info rounded-pill fw-bold shadow-sm px-3 py-1 bg-white" onclick="App.pages.visit_detail.pullPreviousData('xrays')"><i class="fa-solid fa-clock-rotate-left me-1"></i> ดึงประวัติ</button>
                                    </div>
                                    <div id="vd-xrays-container" class="mb-3"></div>
                                    <button class="btn btn-outline-info fw-bold rounded-pill w-100 shadow-sm py-2 bg-white" onclick="App.pages.visit_detail.addXray()">+ เพิ่มรายการ X-Ray</button>
                                </div>
                                ...
                                <div class="modern-panel shadow-sm p-3" style="border-radius: 16px; border: 1px solid #cbd5e1; background-color: #f8fafc;">
                                    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                                        ...
                                        <button class="btn btn-sm btn-success fw-bold rounded-pill shadow-sm px-3" onclick="App.pages.visit_detail.calculateAdditionalFees()"><i class="fa-solid fa-calculator me-1"></i> คำนวณออโต้</button>
                                    </div>
                                    <div class="row g-2">
                                ...
                                <div class="text-end mt-3"><button class="btn btn-premium-warning shadow-sm fw-bold px-4 py-2 w-100 fs-6" id="btn-deduct-stock" onclick="App.pages.visit_detail.deductStock()"><i class="fa-solid fa-box-open me-2"></i> ยืนยันตัดเบิกพัสดุ</button></div>
                            </div>
                        </div>
                            ...
                            <div class="compact-panel modern-panel h-100 position-relative overflow-hidden d-flex flex-column" style="border-top: 5px solid var(--muted);">
                                <div class="section-title-compact text-secondary mb-4">
                                    ...
                                    <div class="d-flex gap-2">
                                        <button class="btn btn-sm btn-outline-danger rounded-pill fw-bold shadow-sm px-3 py-1 bg-white" onclick="App.pages.visit_detail.pullPreviousData('labs')">ดึงแล็บเก่า</button>
                                        <button class="btn btn-sm btn-outline-secondary rounded-pill fw-bold shadow-sm px-3 py-1 bg-white" onclick="App.pages.visit_detail.pullPreviousData('note')">ดึง Note เก่า</button>
                                    </div>
                                </div>
                                ...
                                <div class="p-3 bg-light border border-danger-subtle rounded-4 mb-4 shadow-sm">
                                    <div class="d-flex justify-content-between align-items-center mb-3 border-bottom border-danger-subtle pb-3 flex-wrap gap-2">
                                        ...
                                        <div class="d-flex gap-2">
                                            <select id="vd-lab-set-select" class="form-select form-select-sm shadow-sm fw-bold text-danger input-modern px-3 py-1 w-auto"><option value="">- เลือกชุดแล็บ -</option></select>
                                            <button class="btn btn-sm btn-danger fw-bold shadow-sm rounded-pill px-3" onclick="App.pages.visit_detail.applyLabSet()">โหลด</button>
                                        </div>
                                    </div>
                                    <div id="vd-labs-container" class="row g-2 mb-3"></div>
                                    <button class="btn btn-outline-danger fw-bold rounded-pill w-100 shadow-sm py-2 bg-white" onclick="App.pages.visit_detail.addLabRow()">+ เพิ่มตัวตรวจ (Manual)</button>
                                </div>
                                ...
                                <div class="row g-3 mb-4">
                                    ...
                                    <div class="col-12">
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            ...
                                            <select id="vd-note-template" class="form-select form-select-sm w-auto shadow-sm fw-bold input-modern text-warning-dark px-3 py-1" onchange="App.pages.visit_detail.applyNoteTemplate(this.value)"><option value="">-- เลือกเทมเพลต --</option></select>
                                        </div>
    ...
    destroy() {
        ...
        if (dropzone) {
            dropzone.removeEventListener('dragover', this.boundHandleDragOver);
            dropzone.removeEventListener('dragleave', this.boundHandleDragLeave);
            dropzone.removeEventListener('drop', this.boundHandleDrop);
        }
        console.log("🧹 [Visit Detail] Cleaned up listeners and memory.");
    }

    // 🚨 ลบประวัติ 5 ปีแบบปลอดภัย
    ...
    #showSearchScreen() {
        ...
        setTimeout(() => { 
            ...
            if(dp) { 
                dp.value = this.state.selectedDate; 
            ...
            Promise.all([db.ref('patients_database_v2/visits').once('value'), db.ref('patients_database_v2/patients').once('value')]).then(([vSnap, pSnap]) => {
                ...
                todayVisits.forEach((v, idx) => { 
                    ...
                    <div class="col-md-6 col-lg-4 fade-in-up" style="animation-delay: ${idx * 0.05}s">
                        <div class="modern-panel p-4 h-100 d-flex flex-column card-hover-float" style="border-left: 5px solid ${bColor}; cursor: pointer; ${opacityClass}" onclick="App.pages.visit_detail.init('${v.id}')">
    ...
    #updateDateDisplay(dateStr) { 
        const displayEl = document.getElementById('vdSearchDateDisplay'); 
        if (displayEl && dateStr) { 
            const dateObj = new Date(dateStr); 
            displayEl.innerText = dateObj.toLocaleDateString('th-TH', { day: 'numeric', month: 'long', year: 'numeric' }); 
        } 
    }

    // ---------------------------------------------------------
    // 📋 Screen 2: Flowsheet Detail & Safe Data Load
    // ---------------------------------------------------------
    async #showMainScreen(visitId) {
        ...
        try {
            const [invSnap, medSnap, xraySnap, rightSnap, modSnap, settingsSnap, labSetSnap, noteSnap] = await Promise.all([
                ...
                db.ref('clinic_modalities_v2').once('value'), 
                db.ref('clinic_settings_v2').once('value'), 
                db.ref('clinic_lab_sets_v2').once('value'), 
                db.ref('clinic_note_templates_v2').once('value')
            ]);

            this.state.clinicSettings = settingsSnap.val() || {}; 
            ...
            this.state.medsList = toArray(medSnap.val()); 
            this.state.xraysList = toArray(xraySnap.val()); 
            this.state.clinicRights = toArray(rightSnap.val()); 
            this.state.modalities = toArray(modSnap.val()); 
            this.state.labSets = toArray(labSetSnap.val()); 
            ...
            this.state.noteTemplates = _notes.length > 0 ? _notes : defaultNotes;
            ...
            const ptQuery = await db.ref('patients_database_v2/patients').orderByChild('hn').equalTo(this.state.visitData.hn).once('value');
            if (ptQuery.exists()) {
                ptQuery.forEach(child => {
                    this.state.patientFirebaseKey = child.key; 
                });
            } else {
                console.warn("Patient Profile not found for HN: ", this.state.visitData.hn);
            }

            this.state.currentMeds = Array.isArray(this.state.visitData.other_meds) ? this.state.visitData.other_meds : []; 
            this.state.currentXrays = Array.isArray(this.state.visitData.xray_list) ? this.state.visitData.xray_list : []; 
            this.state.currentLabs = Array.isArray(this.state.visitData.lab_results) ? this.state.visitData.lab_results : []; 
            this.state.currentAttachments = Array.isArray(this.state.visitData.attachments) ? this.state.visitData.attachments : []; 
            ...
            const cbNotes = db.ref('clinic_note_templates_v2').on('value', snap => {
                ...
                if(liveNotes.length > 0) {
                    this.state.noteTemplates = liveNotes;
                    if(this.state.isFormLoaded) this.renderNoteTemplateDropdown();
                }
            });
    ...
    #renderFormUI() {
        ...
        document.getElementById('vd-pt-name').innerText = v.name || '-'; 
        document.getElementById('vd-pt-hn').innerText = v.hn || '-'; 
        document.getElementById('vd-pt-bed-display').innerText = v.bed || '-'; 
        document.getElementById('vd-pt-time').innerText = v.time || '-';
        document.getElementById('vd-status').value = v.status || 'รอตรวจ'; 
        document.getElementById('vd-dialyzer').value = v.hd_dialyzer || '';
        
        this.renderModalityDropdown(); 
        this.renderRightDropdown(); 
        this.refreshDropdownsUI(); 
        this.renderLabSetDropdown(); 
        this.renderNoteTemplateDropdown(); 
        this.renderLabs(); 
        this.renderXrays(); 
        this.renderAttachments();
        
        document.getElementById('vd-mode').value = v.hd_mode || ''; 
        document.getElementById('vd-right').value = v.right || ''; 
        document.getElementById('vd-dialysis-fee').value = v.dialysis_fee !== undefined ? v.dialysis_fee : '';
        
        let defMed = this.state.clinicSettings.med_fee_default || '';
        let defLab = this.state.clinicSettings.lab_fee_default || '';
        let defXray = this.state.clinicSettings.xray_fee_default || '';
    ...
    pullPreviousData(section) {
        Swal.fire('ระบบอยู่ระหว่างการอัปเกรด', 'ฟีเจอร์ดึงประวัติเก่ากำลังอยู่ในช่วงอัปเกรดเป็นระบบ Enterprise ครับ', 'info');
    }

    // ---------------------------------------------------------
    // 🛡️ Data Mutation (Atomic Cloud Synchronization)
    // ---------------------------------------------------------
    async saveData(quietMode = false) {
        if (!this.state.visitFirebaseKey || !this.state.patientFirebaseKey) {
            Swal.fire('ข้อผิดพลาดรุนแรง', 'หาที่อยู่เอกสารบนคลาวด์ไม่พบ กรุณารีเฟรชหน้าเว็บ (Missing DB Keys)', 'error');
            return;
        }

        if(!quietMode) Swal.fire({ title: 'กำลังบันทึก (Atomic Update)...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });

        const visitUpdates = {
            status: document.getElementById('vd-status').value, 
            hd_dialyzer: document.getElementById('vd-dialyzer').value.trim(), 
            hd_mode: document.getElementById('vd-mode').value, 
            right: document.getElementById('vd-right').value, 
            ...
            hd_qb: document.getElementById('vd-qb').value.trim(), 
            hd_qd: document.getElementById('vd-qd').value.trim(), 
            hd_uf: document.getElementById('vd-uf').value.trim(), 
            pre_wt: document.getElementById('vd-pre-wt').value.trim(), 
            post_wt: document.getElementById('vd-post-wt').value.trim(), 
            pre_bp: document.getElementById('vd-pre-bp').value.trim(), 
            post_bp: document.getElementById('vd-post-bp').value.trim(), 
            cc: document.getElementById('vd-cc').value.trim(), 
            note: document.getElementById('vd-note').value.trim(), 
            hd_dialysate_item: document.getElementById('vd-dialysate-item').value, 
            hd_dialysate_qty: document.getElementById('vd-dialysate-qty').value, 
            hd_saline_item: document.getElementById('vd-saline-item').value, 
            hd_saline_qty: document.getElementById('vd-saline-qty').value, 
            hd_heparin_item: document.getElementById('vd-heparin-item').value, 
            hd_heparin_qty: document.getElementById('vd-heparin-qty').value, 
            other_meds: this.state.currentMeds, 
            xray_list: this.state.currentXrays, 
            lab_results: this.state.currentLabs, 
            attachments: this.state.currentAttachments, 
            is_stock_deducted: this.state.isStockDeducted, 
            last_updated: new Date().toISOString(), 
            updated_by: App.currentUser ? App.currentUser.name : 'Unknown'
        };

        try {
            const ptSnap = await db.ref(`patients_database_v2/patients/${this.state.patientFirebaseKey}`).once('value');
            if (ptSnap.exists()) {
                let pt = ptSnap.val();
                ...
                if (!pt.labs) pt.labs = []; 
                if (!pt.medications) pt.medications = [];
                ...
                const getMedName = (id) => { 
                    let f = this.state.inventoryItems.get(String(id)) || this.state.medsList.find(m => String(m.id||m) === String(id)); 
                    return f ? (f.name||f) : 'ไม่ทราบชื่อ'; 
                };
                let newMeds = [];
                if (visitUpdates.hd_dialysate_item) newMeds.push({ name: getMedName(visitUpdates.hd_dialysate_item), dosage: visitUpdates.hd_dialysate_qty ? visitUpdates.hd_dialysate_qty + ' แกลลอน' : '-' });
                if (visitUpdates.hd_saline_item) newMeds.push({ name: getMedName(visitUpdates.hd_saline_item), dosage: visitUpdates.hd_saline_qty ? visitUpdates.hd_saline_qty + ' ขวด' : '-' });
                if (visitUpdates.hd_heparin_item) newMeds.push({ name: getMedName(visitUpdates.hd_heparin_item), dosage: visitUpdates.hd_heparin_qty ? visitUpdates.hd_heparin_qty + ' Vial' : '-' });
                if (Array.isArray(visitUpdates.other_meds)) visitUpdates.other_meds.forEach(m => { if (m.id) newMeds.push({ name: getMedName(m.id), dosage: m.qty ? m.qty + ' หน่วย' : '-' }); });
                pt.medications = newMeds; 

                if (Array.isArray(visitUpdates.lab_results)) {
                    ...
                    if (labIdx !== -1) { 
                        if (hasAnyValue) pt.labs[labIdx] = { ...pt.labs[labIdx], ...labObj }; 
                        else pt.labs.splice(labIdx, 1); 
                    } else if (hasAnyValue) { 
                        pt.labs.push(labObj); 
                    }
                }
                ...
                const finalUpdates = {};
                finalUpdates[`patients_database_v2/visits/${this.state.visitFirebaseKey}`] = { ...this.state.visitData, ...visitUpdates };
                finalUpdates[`patients_database_v2/patients/${this.state.patientFirebaseKey}`] = pt;

                await db.ref().update(finalUpdates);
                
                if(!quietMode) Swal.fire({ title: 'บันทึกสำเร็จ!', text: 'เชื่อมโยงเข้าแฟ้ม EMR แบบ Atomic เรียบร้อย', icon: 'success', timer: 1500, showConfirmButton: false }).then(() => App.switchPage('visits')); 
            }
        } catch(e) {
    ...
    async deductStock() {
                        ...
                        const restorePromises = (this.state.visitData.deducted_log || []).map(async (req) => {
                            ...
                            if(itemQuery.exists()) {
                                itemQuery.forEach(child => {
                                    ...
                                    logsToPush.push({ timestamp, mode: 'in_sub_restore', itemId: req.id, itemName: child.val().name, qty: req.qty, user: App.currentUser?App.currentUser.name:'Admin', note: 'ยกเลิกเบิก HN: ' + this.state.visitData.hn });
                                });
                            }
                        });
        ...
        const addIfValid = (id, qtyStr) => { 
            ...
            if(id && q > 0) { 
                ...
                if(inv) n = inv.name; 
                else { let m = this.state.medsList.find(i=>String(i.id||i)===String(id)); if(m) n = m.name||m; } 
        ...
        this.state.currentMeds.forEach(m => addIfValid(m.id, m.qty));
    ...
    // ---------------------------------------------------------
    // 📸 Camera & Hardware Input Management
    // ---------------------------------------------------------
    #initScannerEvents() {
        const dropzone = document.getElementById('doc-dropzone');
        ...
        if(fileInput) {
            fileInput.onchange = async (e) => {
                if(e.target.files.length > 0) {
                    await this.processScannedFiles(Array.from(e.target.files));
    ...
    #handleDrop(e) {
        ...
        if(e.dataTransfer.files.length > 0) this.processScannedFiles(Array.from(e.dataTransfer.files));
    }
    ...
    renderAttachments() {
        ...
        this.state.currentAttachments.forEach((doc, idx) => {
            ...
            if (doc.type === 'pdf' || (doc.dataUrl && doc.dataUrl.startsWith('data:application/pdf'))) {
                html += `<div class="col-6 col-md-4 fade-in-up" style="animation-delay: ${idx * 0.1}s;"><div class="p-3 border rounded-3 bg-white text-center position-relative shadow-sm h-100 d-flex flex-column align-items-center justify-content-center"><button class="btn btn-sm btn-danger position-absolute shadow-sm delete-btn" style="top:-8px; right:-8px; border-radius:50%; width:30px; height:30px; padding:0; z-index:20;" onclick="App.pages.visit_detail.deleteAttachment('${doc.id}')" title="ลบไฟล์"><i class="fa-solid fa-times"></i></button><i class="fa-solid fa-file-pdf fa-3x text-danger mb-2"></i><div class="small fw-bold text-truncate w-100 mb-2" title="${displayName}">${displayName}</div><button class="btn btn-sm btn-outline-danger rounded-pill px-3 mt-auto" onclick="App.pages.visit_detail.viewAttachment('${doc.dataUrl}', 'pdf')"><i class="fa-solid fa-eye me-1"></i> ดูไฟล์</button></div></div>`;
            } else {
                html += `<div class="col-6 col-md-4 fade-in-up" style="animation-delay: ${idx * 0.1}s;"><div class="scan-img-card h-100 m-0 text-center"><button class="delete-btn shadow-sm" onclick="App.pages.visit_detail.deleteAttachment('${doc.id}')" title="ลบภาพ"><i class="fa-solid fa-times"></i></button><img src="${doc.dataUrl}" onclick="App.pages.visit_detail.viewAttachment('${doc.dataUrl}', 'image')" style="cursor:zoom-in; height:100px; object-fit:cover;"><div class="small text-muted text-truncate w-100 mt-1 fw-bold" style="font-size:11px;" title="${displayName}">${displayName}</div></div></div>`;
            }
        });
        area.innerHTML = html;
    }
    ...
    deleteAttachment(docId) {
        Swal.fire({ title: 'ลบเอกสารนี้?', text:'คุณต้องการลบเอกสารออกจากระบบใช่หรือไม่?', icon: 'warning', showCancelButton: true, confirmButtonText: 'ลบ', confirmButtonColor: '#ef4444' }).then(r => {
            if(r.isConfirmed) { this.state.currentAttachments = this.state.currentAttachments.filter(doc => doc.id !== docId); this.renderAttachments(); }
        });
    }
    ...
    calculateAdditionalFees(quiet = false) {
        ...
        const getPrice = (id) => { 
            if(!id) return 0; 
            let m = this.state.medsList.find(x => String(x.id||x) === String(id)); 
        ...
        this.state.currentMeds.forEach(m => { if(m.id && parseFloat(m.qty) > 0) totalMedFee += getPrice(m.id) * parseFloat(m.qty); });
        this.state.currentXrays.forEach(x => { if(x.id && parseFloat(x.qty) > 0) { let xI = this.state.xraysList.find(i => String(i.id||i) === String(x.id)); if(xI && xI.price) totalXrayFee += parseFloat(xI.price) * parseFloat(x.qty); } });
    ...
    renderModalityDropdown() { const select = document.getElementById('vd-mode'); if(!select) return; let currentVal = select.value || (this.state.visitData ? this.state.visitData.hd_mode : ''); let html = '<option value="">-- เลือกโหมด --</option>'; this.state.modalities.forEach(m => { html += `<option value="${this.#escapeHTML(m.name)}">${this.#escapeHTML(m.name)}</option>`; }); select.innerHTML = html; if(currentVal) select.value = currentVal; }
    renderRightDropdown() { const select = document.getElementById('vd-right'); if(!select) return; let currentVal = select.value || (this.state.visitData ? this.state.visitData.right : ''); let html = '<option value="">-- เลือกสิทธิ --</option>'; this.state.clinicRights.forEach(r => { html += `<option value="${this.#escapeHTML(r.name)}">${this.#escapeHTML(r.name)}</option>`; }); select.innerHTML = html; if(currentVal) select.value = currentVal; }
    onModeChange() { const modeObj = this.state.modalities.find(m => m.name === document.getElementById('vd-mode').value); if (modeObj && modeObj.price > 0) document.getElementById('vd-dialysis-fee').value = modeObj.price; }
    onRightChange() { const rightObj = this.state.clinicRights.find(r => r.name === document.getElementById('vd-right').value); if (rightObj && rightObj.price !== undefined) document.getElementById('vd-dialysis-fee').value = rightObj.price; }
    ...
    refreshDropdownsUI() {
        ...
        if(dEl) { dEl.innerHTML = this.generateMedOptions(dEl.value || this.state.visitData.hd_dialysate_item); dEl.value = this.state.visitData.hd_dialysate_item || ''; }
        if(sEl) { sEl.innerHTML = this.generateMedOptions(sEl.value || this.state.visitData.hd_saline_item); sEl.value = this.state.visitData.hd_saline_item || ''; }
        if(hEl) { hEl.innerHTML = this.generateMedOptions(hEl.value || this.state.visitData.hd_heparin_item); hEl.value = this.state.visitData.hd_heparin_item || ''; }
        this.renderOtherMeds(); 
    }

    generateMedOptions(selectedId) {
        let html = '<option value="">-- เลือกรายการ --</option>';
        if (this.state.medsList.length > 0) { html += '<optgroup label="📋 ยาและเวชภัณฑ์มาตรฐาน">'; this.state.medsList.forEach(m => { html += `<option value="${this.#escapeHTML(m.id||m)}" ${(String(m.id||m) === String(selectedId) ? 'selected' : '')}>${this.#escapeHTML(m.name||m)}</option>`; }); html += '</optgroup>'; }
        
        let allInvItems = Array.from(this.state.inventoryItems.values());
        let categories = [...new Set(allInvItems.map(i => i.category || 'ทั่วไป'))];
        categories.forEach(cat => { 
            let items = allInvItems.filter(i => (i.category || 'ทั่วไป') === cat); 
            if (items.length > 0) { 
                html += `<optgroup label="📦 คลังพัสดุ: ${this.#escapeHTML(cat)}">`; 
                items.forEach(i => { 
                    let stock = (Number(i.qty_sub)||0) + (Number(i.qty_main)||0); 
                    html += `<option value="${this.#escapeHTML(i.id||i)}" ${(String(i.id||i) === String(selectedId) ? 'selected' : '')}>${this.#escapeHTML(i.name||i)} (คงเหลือ: ${stock})</option>`; 
                }); 
                html += '</optgroup>'; 
            } 
        });
    ...
    renderOtherMeds() {
        ...
        let html = ''; this.state.currentMeds.forEach((m, idx) => { 
            let dis = this.state.isStockDeducted ? 'disabled' : ''; 
            html += `<div class="d-flex flex-wrap flex-md-nowrap gap-2 mb-2 align-items-center w-100"><select class="form-select input-modern flex-grow-1 min-w-0 fw-bold text-dark" onchange="App.pages.visit_detail.updateMed(${idx}, 'id', this.value)" ${dis}>${this.generateMedOptions(m.id)}</select><input type="number" class="form-control text-center fw-bold text-primary input-modern flex-shrink-0" style="width: 80px;" placeholder="จำนวน" value="${m.qty||''}" onkeyup="App.pages.visit_detail.updateMed(${idx}, 'qty', this.value)" onchange="App.pages.visit_detail.updateMed(${idx}, 'qty', this.value)" ${dis}><button class="btn btn-light border border-danger-subtle text-danger shadow-sm flex-shrink-0" style="width: 40px; height: 40px; border-radius: 10px; padding:0;" onclick="App.pages.visit_detail.removeMed(${idx})" ${dis}><i class="fa-solid fa-trash"></i></button></div>`; 
        }); 
        container.innerHTML = html;
    }
    ...
    generateXrayOptions(selectedId) { let html = '<option value="">-- เลือกรายการเอ็กซเรย์ --</option>'; this.state.xraysList.forEach(x => { html += `<option value="${this.#escapeHTML(x.id||x)}" ${(String(x.id||x) === String(selectedId) ? 'selected' : '')}>${this.#escapeHTML(x.name||x)}</option>`; }); return html; }
    renderXrays() {
        ...
        let html = ''; this.state.currentXrays.forEach((x, idx) => { html += `<div class="d-flex flex-wrap flex-md-nowrap gap-2 mb-2 align-items-center w-100"><select class="form-select input-modern flex-grow-1 min-w-0 fw-bold text-dark" onchange="App.pages.visit_detail.updateXray(${idx}, 'id', this.value)">${this.generateXrayOptions(x.id)}</select><input type="number" class="form-control text-center fw-bold text-info input-modern flex-shrink-0" style="width: 80px;" placeholder="รอบ" value="${x.qty||''}" onkeyup="App.pages.visit_detail.updateXray(${idx}, 'qty', this.value)" onchange="App.pages.visit_detail.updateXray(${idx}, 'qty', this.value)"><button class="btn btn-light border border-danger-subtle text-danger shadow-sm flex-shrink-0" style="width: 40px; height: 40px; border-radius: 10px; padding:0;" onclick="App.pages.visit_detail.removeXray(${idx})"><i class="fa-solid fa-trash"></i></button></div>`; }); container.innerHTML = html;
    }
    ...
    renderNoteTemplateDropdown() { const select = document.getElementById('vd-note-template'); if(!select) return; let html = '<option value="">-- เลือกเทมเพลต --</option>'; const catMap = { 'pre': 'ก่อนฟอก', 'intra': 'แทรกซ้อน', 'post': 'หลังฟอก', 'doctor': 'คำสั่งแพทย์', 'general': 'ทั่วไป' }; this.state.noteTemplates.forEach(t => { html += `<option value="${this.#escapeHTML(t.id)}">[${this.#escapeHTML(catMap[t.category]||t.category||'ทั่วไป')}] ${this.#escapeHTML(t.title || t.name)}</option>`; }); select.innerHTML = html; }
    applyNoteTemplate(id) { if(!id) return; const t = this.state.noteTemplates.find(x => String(x.id) === String(id)); if(t) { const noteEl = document.getElementById('vd-note'); let textToAppend = (t.text || '').replace(/\\n/g, '\n'); noteEl.value = noteEl.value.trim() ? noteEl.value.trim() + '\n\n' + textToAppend : textToAppend; } document.getElementById('vd-note-template').value = ''; }

    renderLabSetDropdown() { const select = document.getElementById('vd-lab-set-select'); if(!select) return; let html = '<option value="">- เลือกชุดแล็บ -</option>'; this.state.labSets.forEach(s => html += `<option value="${this.#escapeHTML(s.id)}">${this.#escapeHTML(s.name)}</option>`); select.innerHTML = html; }
    applyLabSet() {
        const setId = document.getElementById('vd-lab-set-select').value; if(!setId) return; const setObj = this.state.labSets.find(s => String(s.id) === String(setId)); if(!setObj) return;
        ...
        setObj.items.forEach(i => { if(!this.state.currentLabs.some(l => l.name === i)) this.state.currentLabs.push({ name: i, value: '' }); });
