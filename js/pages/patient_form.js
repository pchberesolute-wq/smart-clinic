...
// 🚀 Enterprise Patient Form Module: Encapsulated ES6 Class & Real-time Master Data Sync
...
class PatientFormPageComponent {
        ...
        return `
            ...
            <div class="page-header mb-4">
                ...
                <div class="d-flex gap-2 mt-3 mt-md-0">
                    <button class="btn btn-premium btn-premium-success px-5 py-2 shadow-sm" style="font-size: 16px;" onclick="App.pages.patient_form.saveData()">
                ...
                <div class="col-md-3">
                    <div class="modern-panel text-center h-100 position-sticky shadow-sm p-4" style="top: 100px;">
                        ...
                        <div id="smartcard-section" class="position-relative z-1">
                            ...
                            <button class="btn btn-premium btn-premium-primary w-100 mb-3 py-2 shadow-sm" id="btn-read-card" onclick="App.pages.patient_form.readSmartcard()">
    ...
    init(hnToEdit = null) {
        ...
        const cbRights = db.ref('clinic_rights_v2').on('value', snap => {
            if(!document.getElementById('add_right')) return;
            const data = snap.val();
            if(data) {
                this.state.clinicRights = Array.isArray(data) ? data : Object.keys(data).map(k => data[k]);
                let rightsOptions = this.state.clinicRights.map(r => `<option value="${this.#escapeHTML(r.name)}">${this.#escapeHTML(r.name)}</option>`).join('');
        ...
        db.ref('clinic_options_v2').once('value', snap => {
            ...
            const buildOptions = (arr) => arr.map(a => `<option value="${this.#escapeHTML(a)}">${this.#escapeHTML(a)}</option>`).join('');
            
            if(document.getElementById('add_shift')) document.getElementById('add_shift').innerHTML = buildOptions(opts.shifts);
            if(document.getElementById('add_infection')) document.getElementById('add_infection').innerHTML = buildOptions(opts.infections);
            if(document.getElementById('underlying_list')) document.getElementById('underlying_list').innerHTML = buildOptions(opts.underlyings);
            ...
            if (hnToEdit) {
                ...
                if (typeof App.pages.patients !== 'undefined' && App.pages.patients.state && App.pages.patients.state.allData) {
                    const pt = App.pages.patients.state.allData.find(p => p.hn === hnToEdit);
                    if (pt) {
                        this.state.editOriginalData = pt;
                        ...
                        setTimeout(() => {
                            ...
                            if(rightEl) {
                                ...
                                if(!hasOption && pt.right) rightEl.innerHTML += `<option value="${this.#escapeHTML(pt.right)}">${this.#escapeHTML(pt.right)}</option>`;
                                rightEl.value = pt.right || "";
                            }
                        }, 500);

                        if(!opts.shifts.includes(pt.shift) && pt.shift) document.getElementById('add_shift').innerHTML += `<option value="${this.#escapeHTML(pt.shift)}">${this.#escapeHTML(pt.shift)}</option>`;
                        if(!opts.infections.includes(pt.infection) && pt.infection) document.getElementById('add_infection').innerHTML += `<option value="${this.#escapeHTML(pt.infection)}">${this.#escapeHTML(pt.infection)}</option>`;
                        ...
                        if (pt.photo_base64) {
                            this.state.scannedPhotoBase64 = pt.photo_base64;
    ...
    async readSmartcard() {
        ...
        btn.innerHTML = `<i class="fas fa-spinner fa-spin me-2"></i> กำลังสื่อสาร...`; 
        btn.disabled = true;
        ...
        try {
            ...
            if (result.error || result.status === "error") { 
                ...
                btn.innerHTML = `<i class="fa-solid fa-id-card me-2"></i> สแกนบัตร ปชช.`; 
                btn.disabled = false; 
                return; 
            }
            ...
            btn.innerHTML = `<i class="fa-solid fa-check-circle me-2"></i> สำเร็จ`; 
            btn.className = "btn btn-premium btn-premium-success w-100 py-2 mb-3 shadow-sm"; 
            btn.disabled = false;
        } catch (err) {
            Swal.fire('ไม่พบตัวเชื่อมต่ออุปกรณ์', 'เปิดโปรแกรม Local Bridge Agent ไว้หรือยัง?', 'warning'); 
            btn.innerHTML = `<i class="fa-solid fa-id-card me-2"></i> สแกนบัตร ปชช.`; 
            btn.disabled = false;
        }
    }
    ...
    saveData() {
        const hn = document.getElementById('add_hn').value.trim(); 
        ...
        db.ref('patients_database_v2/patients').once('value', snap => {
            ...
            db.ref('patients_database_v2/patients').set(currentList)
                .then(() => { 
                    Swal.fire({ title: 'สำเร็จ!', text: `บันทึกข้อมูลเรียบร้อย`, icon: 'success', confirmButtonColor: '#10b981' })
                    .then(() => { App.switchPage('patients'); }); 
                })
