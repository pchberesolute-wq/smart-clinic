...
// 🚀 Enterprise Patient Form Module: HTTP Cache-Busting, Zero-Trust Clear, Smart Timeout & Premium OPD Print Engine (v7.2)
...
class PatientFormPageComponent {
    ...
    // 🚨 THE FIX: ระบบสแกนทะลวง Cache พร้อม Smart Timeout Alert (15s Circuit Breaker)
    async readSmartcard() {
        ...
        // 🚨 1. สร้างตัวตัดไฟ (AbortController)
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 15000); // 15 วินาที ตัดจบ!

        try {
            const timestamp = new Date().getTime();
            const url = `http://127.0.0.1:8000/read-card?_t=${timestamp}`;

            // 🚨 2. ผูกสายชนวนตัดไฟเข้ากับคำสั่ง fetch
            ...
            clearTimeout(timeoutId); // อ่านเสร็จก่อน ถอดชนวนทิ้ง
            ...
            if (result.error || result.status === "error") { 
                Swal.fire('Error', result.error, 'error', {customClass: {popup: 'premium-alert'}}); 
            ...
            clearTimeout(timeoutId); // ป้องกันบั๊กชนวนค้าง

            // 🚨 3. วิเคราะห์สาเหตุ ว่าดึงไม่รอด หรือ หมดเวลา
            if (err.name === 'AbortError') {
                ...
                Swal.fire({
                    title: 'ไม่พบเครื่องอ่านบัตร ปชช.', 
                    html: 'กรุณาตรวจสอบว่า:<br>1. เสียบสาย USB แล้ว<br>2. เปิดโปรแกรม <b>Local Bridge Agent</b> แล้ว', 
                    icon: 'warning',
                    customClass: { popup: 'premium-alert' }
                }); 
            }
    ...
    saveData() {
        ...
        db.ref('patients_database_v2/patients').once('value', snap => {
            ...
            if (this.state.isEditMode && this.state.editOriginalData) {
                ...
                if(currentList.some(p => p.hn === hn)) {
                    Swal.fire('ข้อผิดพลาด', `รหัสผู้ป่วย ${hn} มีในระบบแล้ว กรุณาใช้รหัสอื่น`, 'error');
                    return;
                }
                    ...
                    <div class="page-half back-cover">
                        ...
                        <div class="clinic-branding">
                            ...
                            <div class="clinic-info">
                                ...
                                <strong>เลขประจำตัวผู้เสียภาษี:</strong> ${this._escapeHTML(clinic.tax_id || '-')}<br><br>
                        ...
                        <div class="footer-note">พิมพ์จากระบบ Dialysis Pro EMR System (Engine v7.2 - Premium UI Layout) เมื่อ ${new Date().toLocaleDateString('th-TH')} เวลา ${new Date().toLocaleTimeString('th-TH')} น.</div>
                    </div>
